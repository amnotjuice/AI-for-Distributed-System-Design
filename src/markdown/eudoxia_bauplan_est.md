# Eudoxia + Bauplan — Context for Policy Generation (with Estimator)
...
* In this configuration, Eudoxia also runs an **estimator** that provides memory hints to the scheduler before assignment.
...
Eudoxia includes a modular **estimator** that runs alongside the simulator and writes scheduling-visible hints onto each operator before the scheduler sees it.

### What it provides

Each `Operator` object has an `estimate` field of type `Estimate`:

```python
op.estimate.mem_peak_gb  # float or None — estimated peak memory usage in GB
```

- **`mem_peak_gb`**: the estimated peak RAM (in GB) that this operator will consume at runtime.
  Always check for `None` before using — it is `None` when no estimator is configured.
  When the `"noisy"` estimator is active, this field is always populated before the scheduler is called.

### How it works (noisy estimator)

The `"noisy"` estimator computes the true peak memory from the operator's segments, then applies lognormal multiplicative noise:

```
estimated_mem = true_mem_peak × lognormal(mean=0, sigma=σ)
```

- `sigma=0.0` → **oracle**: estimate equals the true peak (perfect information).
- `sigma=0.5` → low noise: estimate is typically within ~60% of the true value.
- `sigma=1.0` → medium noise: estimate can be off by a factor of ~2–3×.
- `sigma=1.5` → high noise: estimate may be significantly over- or under-estimated.

The noise is multiplicative and always positive, so the estimate is always ≥ 0.

### How to use it in a scheduler

```python
for op in assignable_ops:
    est_mem = op.estimate.mem_peak_gb  # may be None if no estimator
    if est_mem is not None:
        # skip pools where the operator likely won't fit
...
- **Pre-filter pools**: skip assignment to pools where `est_mem > avail_ram_pool` to reduce OOM failures.
- **Sort by memory footprint**: assign memory-light operators first to keep headroom for large ones.
- **Adaptive fallback**: if `mem_peak_gb` is None or very large, fall back to conservative sizing.
- **Treat as a hint**: the estimate is noisy — do not rely on it exclusively; combine with observed OOM signals.
...
* Memory-aware (with estimator): use `op.estimate.mem_peak_gb` to pre-filter infeasible placements, sort by estimated memory, and avoid OOM cascades.
...
* Memory estimation: use `op.estimate.mem_peak_gb` to anticipate OOM failures before they happen.
