...
def scheduler_low_001_init(s):
    ...
    # Pipeline-level RAM hedge (bounded, mild; avoid crowding out small clusters)
    ...
    # Allow more intra-pipeline parallelism to reduce incomplete pipelines
    ...
    # Headroom reservation (smaller than before to stay work-conserving on small clusters)
    s.reserve_frac_for_query = 0.04
    s.reserve_frac_for_interactive = 0.02

    # Aging thresholds (in ticks) to prevent starvation (important due to incomplete penalties)
    s.age_interactive_to_query = 60
    s.age_batch_to_interactive = 45
    s.age_batch_to_query = 140

    # Panic mode: if waited too long, relax reservations and scale RAM faster to finish
    s.panic_wait_ticks = 180

    # Candidate scan limits
    s.scan_limit_per_pick = 160

    # Preemption (best-effort; only if we can find running container ids)
    s.max_preemptions_per_tick = 10
...
def _base_cpu_for(pool, eff_prio, avail_cpu=None) -> float:
    ...
    # Smaller defaults to increase concurrency on small clusters (improves completion rate).
    if eff_prio == Priority.QUERY:
        cpu = 0.06 * max_cpu
        cpu = _clamp(cpu, 2.0, 10.0)
    elif eff_prio == Priority.INTERACTIVE:
        cpu = 0.045 * max_cpu
        cpu = _clamp(cpu, 1.5, 8.0)
    else:
        cpu = 0.03 * max_cpu
        cpu = _clamp(cpu, 1.0, 6.0)

    # If pool is very idle, scale up to cut tail latency.
...
def _base_ram_for(pool, eff_prio) -> float:
    ...
    # Lower base RAM to allow higher RAM utilization (more concurrent ops) on small clusters.
    # Use a small fraction of pool RAM but keep a minimum.
    if eff_prio == Priority.QUERY:
        base = max(3.0, min(28.0, 0.020 * max_ram))
        mult = 1.10
    elif eff_prio == Priority.INTERACTIVE:
        base = max(2.5, min(24.0, 0.018 * max_ram))
        mult = 1.06
    else:
        base = max(2.0, min(20.0, 0.015 * max_ram))
        mult = 1.03

    ram = base * mult

    # Don't take an extreme share by default; leave room for other ops and retries.
    cap = max_ram * 0.55
    if ram > cap:
        ram = cap
...
def _min_headroom_for_one_query(s, pool):
    ...
    ram = max(3.0, min(0.035 * max_ram, 64.0))
...
def _min_headroom_for_one_interactive(s, pool):
    ...
    cpu = _clamp(0.045 * max_cpu, 1.5, 8.0)
    ram = max(2.5, min(0.030 * max_ram, 48.0))
    return float(cpu), float(ram)
...
def _reserve_for_higher_prio(s, pool, any_query_runnable: bool, any_interactive_runnable: bool):
    ...
    # Keep reservations bounded to stay work-conserving.
    reserve_cpu = min(reserve_cpu, max_cpu * 0.18)
    reserve_ram = min(reserve_ram, max_ram * 0.18)

    return float(reserve_cpu), float(reserve_ram)


def _compute_request(s, pool, pid, base_prio, eff_prio, op, avail_cpu=None, avail_ram=None):
    try:
        max_ram = float(pool.max_ram_pool)
    except Exception:
        max_ram = 1.0
    ...
    # Safety factor: small; most convergence should come from OOM-driven backoff.
    safety = 1.04
    if eff_prio == Priority.QUERY:
        safety = 1.06
    elif eff_prio == Priority.INTERACTIVE:
    ...
    if ooms and int(ooms) > 0:
        # After OOM(s), jump faster; avoid repeated OOM loops that lead to incompletes.
        safety *= min(1.55, 1.12 + 0.12 * float(int(ooms)))
    ...
    if last_good and float(last_good) > 0.0 and (not ooms or int(ooms) <= 0):
        ram = max(ram, float(last_good) * 0.98)

    waited = int(s.tick - int(s.last_scheduled_tick.get(pid, s.tick)))
    if waited >= int(s.panic_wait_ticks):
        # Finish mode: allocate enough to avoid further OOMs; prioritize completion.
        ram = max(ram, max_ram * 0.60)
        cpu = max(cpu, _clamp(0.08 * float(getattr(pool, "max_cpu_pool", 1.0)), 2.0, 16.0))

    ram *= safety

    # Respect learned upper bound if present, but never below lo.
    if hi is not None:
        try:
            hif = float(hi)
            if hif > 0.0:
                soft_hi = hif * 0.995
                if lo and float(lo) > 0.0:
                    soft_hi = max(soft_hi, float(lo) * 1.01)
                ram = min(ram, soft_hi)
        except Exception:
            pass

    # Final caps: leave some slack for other work.
    max_cap = max_ram * 0.94
    if ram > max_cap:
        ram = max_cap
    if ram < float(s.min_ram):
        ram = float(s.min_ram)

    # If we know availability, avoid requesting impossibly large RAM (still allow big if possible).
...
def _pick_runnable(s, stage_prio, scheduled_count_by_pid, assigned_op_keys_by_pid):
    ...
    for qprio in candidate_queues:
        ...
        scanned = 0
        scan_limit = min(int(s.scan_limit_per_pick), n) if n > 0 else 0
        ...
        for i in range(n):
            ...
            # Bias: waited and age drive completion; base priority breaks ties.
            ...
            fail_bump = min(160, 12 * int(s.failed_seen_count.get(pid, 0)))

            # Prefer older pipelines to reduce incompletes; waited tracks "lack of progress".
            score = waited * 11 + age * 2 + base_bias + fail_bump
...
def _attempt_scheduling_pass(
    ...
    # Weighted stage sequence: keep batch flowing to avoid incomplete penalties, while protecting tail for queries.
    prio_seq = [Priority.QUERY] * 10 + [Priority.INTERACTIVE] * 5 + [Priority.BATCH_PIPELINE] * 5
    ...
    while total_made < int(s.max_total_assignments_per_tick):
        ...
        for _ in range(len(prio_seq)):
            ...
            # Pool iteration order (if multiple pools)
            ...
            for pool_id in pool_ids:
                ...
                cpu_req, ram_req = _compute_request(
                    s, pool, pid, base_prio, eff_prio, op, avail_cpu=avail_cpu, avail_ram=avail_ram
                )
                ...
                # Reservations (soft): apply only when not panic and not disabled.
                if (not disable_reservations) and (not panic):
                    if eff_prio == Priority.BATCH_PIPELINE:
                        ...
                        # Relax reserves with waiting to avoid starvation.
                        relax = 0.0
                        if waited >= int(s.age_batch_to_interactive):
                            relax = 0.40
                        if waited >= int(s.age_batch_to_query):
                ...
                # Packing score: prefer tighter RAM fit; mild CPU term.
                score = (avail_ram - ram) + 0.01 * (avail_cpu - cpu)
            ...
            assignments.append(
                Assignment(
                    ...
                    cpu=float(best_cpu),
                    ram=float(best_ram),
                    priority=eff_prio,  # aged promotions get urgency in executor too
...
def scheduler_low_001_scheduler(s, results, pipelines):
        ...
        try:
            ...
            if is_failed:
                ...
                if _is_oom_error(getattr(r, "error", None)):
                    # Mild pipeline boost: keep bounded to avoid collapsing concurrency on small clusters.
                    cur = float(s.pipeline_ram_boost.get(pid, 1.0))
                    nxt = max(cur * 1.20 + 0.05, cur + 0.25)
                    ...
                    if ok is not None:
                        ...
                        # OOM => required > last_alloc
                        lo = max(float(lo), last_alloc * 1.50)

                        # If hi conflicts, drop it.
                        if hi is not None:
                            try:
                                if float(hi) < lo:
                                    hi = None
                        ...
                        # Jump estimate aggressively to converge in few retries.
                        if est is None or float(est) <= 0.0:
                            est = max(lo * 1.08, last_alloc * 2.40)
                        else:
                            est = max(float(est) * 2.10, last_alloc * 2.40, lo * 1.05)

                        s.op_ram[ok] = (lo, hi, est, ooms, float(last_good))
                else:
                    # Non-OOM failures: small increase to reduce repeated failures.
                    cur = float(s.pipeline_ram_boost.get(pid, 1.0))
                    if cur < 2.5:
                        s.pipeline_ram_boost[pid] = min(2.5, cur * 1.10 + 0.02)
            else:
                # Success: decay pipeline boost quickly to restore concurrency.
                cur = float(s.pipeline_ram_boost.get(pid, 1.0))
                if cur > 1.0:
                    s.pipeline_ram_boost[pid] = max(1.0, cur * 0.985)
                ...
                if ok is not None:
                    ...
                    # Decay estimate toward successful allocation, but not below lo.
                    if est is None or float(est) <= 0.0:
                        est = last_alloc
                    else:
                        est = float(est) * 0.88
                    if hi is not None:
                        try:
                            est = min(float(est), float(hi) * 0.99)
                        except Exception:
                            pass
                    if lo > 0.0:
                        est = max(float(est), lo * 1.03)

                    # Track last good allocation for non-OOM path.
    ...
    any_query_runnable = _any_runnable_in_queue(s, Priority.QUERY, scan_limit=96)
    any_interactive_runnable = _any_runnable_in_queue(s, Priority.INTERACTIVE, scan_limit=96)
    ...
    # Work-conserving fallback
    if s.enable_work_conserving_fallback:
        any_batch_runnable = _any_runnable_in_queue(s, Priority.BATCH_PIPELINE, scan_limit=96)
        if (any_query_runnable or any_interactive_runnable or any_batch_runnable) and (len(assignments) == 0):
            assignments.extend(
                _attempt_scheduling_pass(
                    s,
                    any_query_runnable=any_query_runnable,
